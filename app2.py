...
def generate_map_and_report(latitude, longitude):
    ...
    map_element = html.Img(src='data:image/png;base64,{}'.format(img_base64), style={'height': '500px', 'width': 'auto'})
   ...
   # Convert POIs to DataFrame
    ...
    # Save report as PDF
    report_filename = 'Nervs_Report.pdf'
    with PdfPages(report_filename) as pdf:
        pdf.savefig(fig)
        for i, chart in enumerate(chart_elements):
            plt.figure(figsize=(10, 6))
            ax = charts[i][1].value_counts().plot(kind='bar')
            for p in ax.patches:
                ax.annotate(str(p.get_height()), (p.get_x() + p.get_width() / 2., p.get_height()),
                            ha='center', va='center', xytext=(0, 10), textcoords='offset points')
            plt.title('{}'.format(charts[i][0]))
            plt.xlabel('Building Use' if charts[i][0].startswith('Chart showing Use') else 'Road Condition' if charts[i][0].startswith('Chart showing condition') else 'Road Class' if charts[i][0].startswith('Chart showing road') else 'Number of floors')
            plt.ylabel('Count')
            plt.xticks(rotation=45)
            plt.annotate('This report was generated by Nervs', (0.75, 0.05), xycoords='figure fraction', ha='left', fontsize=6.5)
            plt.tight_layout()
    ...
    # Return map, chart elements, and PDF filename
    return map_element, html.Div(style={'display': 'flex', 'justify-content': 'center'}, children=[
        dash_table.DataTable(
            id='poi-table',
            columns=[{"name": i, "id": i} for i in pois_df.columns],
            data=pois_df.to_dict('records'),
            style_table={'overflowX': 'auto', 'color': '#000', 'width': '100%', 'height': '300px'},
            style_cell={'backgroundColor': '#fff', 'color': '#000'}
        )
    ]), *chart_elements, html.A("Download Report", href=f'/{report_filename}', download=report_filename, style={'color': '#fff', 'textDecoration': 'none', 'marginTop': '20px', 'backgroundColor': '#007bff', 'padding': '10px 20px', 'borderRadius': '5px'})
...
def run_analysis(n_clicks, latitude, longitude):
    if n_clicks > 0:
        return generate_map_and_report(latitude, longitude)
